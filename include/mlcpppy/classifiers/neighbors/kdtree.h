...
class KDTree : public NearestNeighbor<T, N> {
       private:
        ...
        Node* Build(std::vector<Point<T, N>> points, int depth) {
                ...
                std::sort(points.begin(), points.end(),
                          [axis](const Point<T, N>& a, const Point<T, N>& b) {
                                  return a.data()[axis] < b.data()[axis];
                          });

                int median = points.size() / 2;
                std::vector<Point<T, N>> points_left(points.begin(),
                                                     points.begin() + median);
        ...
        Node* NearestNeighbor(Node* root, Point<T, N>& target, int depth) {
                ...
                double dist =
                    target.data().at(axis) - root->point_.data().at(axis);
        ...
        static void Inorder(Node* root) {
                ...
                if (root->left_) {
                        ...
                        for (size_t i = 0; i < root->point_.size(); i++)
                                std::cout
                                    << root->point_.data()[i]
                                    << (i + 1 == root->point_.size() ? ""
                                                                     : ",");
                        std::cout << "\" -> \"";
                        for (size_t i = 0; i < root->left_->point_.size(); i++)
                                std::cout
                                    << root->left_->point_.data()[i]
                                    << (i + 1 == root->left_->point_.size()
                                            ? ""
                                            : ",");
                        std::cout << "\" [label=\"esq\"];\n";
                }
       ...
       public:
        ...
        std::vector<Point<T, N>> KNearestNeighbor(Point<T, N> target_points,
                                                  int k) override {
                if (k) this->K_ = k;

                if (k == 1) {
                        Node* result =
                            NearestNeighbor(this->root_, target_points, 0);
        ...
        void PrintInorder() {
                // TODO: A ideia é isso gerar um arquivo para o graphiz gerar o
                // grafo
                std::cout << "digraph G {\n";
                Inorder(this->root_);
                std::cout << "}\n";
